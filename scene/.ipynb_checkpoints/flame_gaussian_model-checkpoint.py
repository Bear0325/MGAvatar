...
class FlameGaussianModel(GaussianModel):
  ...
  # 4. 获取姿态依赖的高斯资产（精炼版本）
    ...
    def select_mesh_by_timestep(self, timestep, original=False):
        ...
        # 5. 添加SMPLX表情偏移
        # smplx facial expression offset
        # smplx_expr_offset_mesh = (flame_param['expr'][[timestep]].squeeze(0)[None,None,:] * self.expr_dirs).sum(2) # 计算表情引起的顶点偏移：expr参数 × 表情形变基
        # mean_3d_mesh = mean_3d_mesh + smplx_expr_offset_mesh
        # mean_3d_refined_mesh = mean_3d_refined_mesh + smplx_expr_offset_mesh
