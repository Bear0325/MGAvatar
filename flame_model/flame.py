...
class FlameHead(nn.Module):
    ...
    def __init__(
        ...
        exprdirs = self.shapedirs[:, :, self.n_shape_params:self.n_shape_params+self.n_expr_params]
        self.register_buffer("exprdirs", exprdirs)

        self.vertex_num = self.v_template.shape[0]
        self.joint_num = self.J_regressor.shape[0]
        self.expr_vertex_idx = self.get_expr_vertex_idx(flame_model_path)
        
        # subdivider
        self.subdivider_list = self.get_subdivider(1)
        # self.face_upsampled = self.subdivider_list[-1]._subdivided_faces.cpu().numpy()

        if len(self.subdivider_list) > 0:
            self.face_upsampled = self.subdivider_list[-1]._subdivided_faces.cpu().numpy()
        else:
            self.face_upsampled = self.faces.detach().cpu().numpy().copy()

        self.vertex_num_upsampled = int(np.max(self.face_upsampled)+1)
    
    def get_expr_vertex_idx(self, flame_model_path):
        # FLAME 2020 has all vertices of expr_vertex_idx. use FLAME 2019
        flame_model_dir = osp.dirname(flame_model_path)
        with open(osp.join(flame_model_dir, '2019_generic_model.pkl'), 'rb') as f:
            flame_2019 = pickle.load(f, encoding='latin1')
        vertex_idxs = np.where((flame_2019['shapedirs'][:,:,300:300+self.n_expr_params] != 0).sum((1,2)) > 0)[0] # FLAME.SHAPE_SPACE_DIM == 300
    ...
    def get_subdivider(self, subdivide_num):
        ...
        for i in range(subdivide_num):
            ...
            subdivider_list.append(subdivider)
            mesh = subdivider(mesh)

        return subdivider_list
    # def upsample_mesh(self, vert, feat_list=None):
    #     # face = torch.LongTensor(self.faces).cuda()
    #     face = torch.as_tensor(self.faces, dtype=torch.long, device=vert.device)
    #     mesh = Meshes(vert[None,:,:], face[None,:,:])
