...
def compute_face_orientation(verts, faces, return_scale=False):
    i0 = faces[..., 0].long() # 1. 取出每个三角面对应的三个顶点索引
    i1 = faces[..., 1].long()
    i2 = faces[..., 2].long()

    v0 = verts[..., i0, :] # 1. 取出每个三角面对应的三个顶点索引
    v1 = verts[..., i1, :]
    v2 = verts[..., i2, :]

    a0 = safe_normalize(v1 - v0) # 3. 构造局部坐标系的第一根轴 a0（切线方向）
    a1 = safe_normalize(torch.cross(a0, v2 - v0, dim=-1)) # 4. 构造局部坐标系的第二根轴 a1
    a2 = -safe_normalize(torch.cross(a1, a0, dim=-1))    # 5. 构造局部坐标系的第三根轴 a2（法线方向）

    orientation = torch.cat([a0[..., None], a1[..., None], a2[..., None]], dim=-1)   # 表示从“局部三角面坐标系”到“世界坐标系”的旋转

    if return_scale:
        s0 = length(v1 - v0) # 三角面第一条边的长度
        s1 = dot(a2, (v2 - v0)).abs() # 第三顶点在法线方向上的投影长度
        scale = (s0 + s1) / 2 #表示的是三角面的大小，用于高斯点局部缩放，与高斯点的scale属性不一样
