...
class PositionMapGenerator:
    ...
    def generate_position_map(
    ...
    ) -> torch.Tensor: #生成 UV 空间下的 Position Map-每个 UV 像素存储对应的 3D 顶点坐标

        vertices = self.normalize_vertices(self.verts) # 对输入顶点进行归一化

        self.position_map = self.render_uv.world2uv(vertices).detach() # 将 3D 世界空间顶点投影/烘焙到 UV 空间
        ...
        return self.position_map  # 返回 UV position map
    ...
    def displacement_map(
        ...
        verts = self.normalize_vertices(verts)   # 对顶点坐标进行归一化
        position_map = self.render_uv.world2uv(verts).detach()  # 将3D顶点投影到UV空间

        displacement_map = position_map - self.position_map   # 计算位移图
...
class SRenderY(nn.Module):
    def __init__(
        ...
        super(SRenderY, self).__init__()  # 初始化父类 nn.Module
        self.image_size = image_size    # 最终图像渲染分辨率
        self.uv_size = uv_size   # UV 空间分辨率
        if rasterizer_type == "pytorch3d":    # 如果使用 pytorch3d 光栅化器
            self.rasterizer = Pytorch3dRasterizer(image_size)  # 创建 image-space rasterizer
            self.uv_rasterizer = Pytorch3dRasterizer(uv_size)  # 创建 UV-space rasterizer
        else:
            NotImplementedError

        # faces
        dense_triangles = generate_triangles(uv_size, uv_size)   # 在 UV 网格上生成规则三角面
        self.register_buffer(  # 注册 dense_faces 为 buffer
            "dense_faces", torch.from_numpy(dense_triangles).long()[None, :, :]
        )
        self.register_buffer("faces", faces)   # 注册 mesh 三角面索引
        self.register_buffer("raw_uvcoords", uvcoords)  # 保存原始 UV 坐标

        # uv coords
        uvcoords = torch.cat(  # 给 UV 坐标补第三维
            [uvcoords, uvcoords[:, :, 0:1] * 0.0 + 1.0], -1
        )  # [bz, ntv, 3]
        uvcoords = uvcoords * 2 - 1   # 将 UV 坐标从 [0,1] 映射到 [-1,1]
        uvcoords[..., 1] = -uvcoords[..., 1]   # 翻转 y 轴
        face_uvcoords = face2vertices(uvcoords, uvfaces)  # 根据 uvfaces 获取每个 triangle 对应的 UV 顶点
        self.register_buffer("uvcoords", uvcoords) # 保存处理后的 UV 坐标
        self.register_buffer("uvfaces", uvfaces) # 保存 UV face 索引
        self.register_buffer("face_uvcoords", face_uvcoords) # 保存 face 对应的 UV 顶点坐标
        ...
        face_colors = face2vertices(colors, faces)   # 将 vertex color 转换为 per-face vertex color
        self.register_buffer("face_colors", face_colors)  # 保存 face color
        ...
        self.register_buffer("constant_factor", constant_factor)  # 保存 SH 常数系数
